...
IMAGES = {
    "gateway": ("images/paid-gateway", "ghcr.io/aptos-labs/confidential-qwen-minimax-paid-gateway", 4),
    "qwen": ("images/qwen-metered", "ghcr.io/aptos-labs/confidential-qwen-minimax-qwen-metered", 24),
}
DIGEST = "sha256:" + "a" * 64
...
def run_shell(script, directory, env=None):
    ...
    return subprocess.run(["bash", "-c", script], cwd=directory, env=variables,
...
class WorkflowPolicyTests(unittest.TestCase):
    ...
    def test_triggers_are_pr_and_explicit_reviewed_dispatch_only(self):
        ...
        self.assertEqual(self.publish["env"]["SOURCE_URL"],
    ...
    def test_workflow_and_job_permissions_are_least_privilege(self):
        ...
        for job in self.validate["jobs"].values():
            self.assertEqual(job.get("permissions", self.validate["permissions"]), {"contents": "read"})
            self.assertNotIn("secrets.", json.dumps(job))
        for kind in IMAGES:
            self.assertEqual(self.publish["jobs"][kind]["permissions"], {
                "contents": "read", "packages": "write", "id-token": "write", "attestations": "write",
            })
        self.assertEqual(self.publish["jobs"]["anonymous_pull"]["permissions"], {"contents": "read"})
    ...
    def test_image_jobs_are_isolated_on_standard_runners_with_timeouts(self):
        self.assertEqual(set(self.validate["jobs"]), {"gateway", "qwen"})
        ...
        for workflow in self.workflows():
            ...
            for job in workflow["jobs"].values():
                ...
                for step in job["steps"]:
                    ...
                    script = step.get("run", "")
                    self.assertNotIn("${{", script, "Pass expressions through env, not shell interpolation")
                    self.assertNotRegex(script, r"\|\|\s*true|set\s+\+e")
                    self.assertNotRegex(script, r"\b(gh\s+(workflow|release|api)|git\s+push|tinfoil\s+deploy)\b")
    ...
    def test_disk_gate_runs_before_buildx_and_building(self):
        for workflow in self.workflows():
            for kind in IMAGES:
                ...
                self.assertLess(job["steps"].index(gate),
                                job["steps"].index(action_steps(job, "docker/setup-buildx-action")[0]))
                self.assertLess(job["steps"].index(gate), job["steps"].index(step_by_id(job, "test")))
        ...
        self.assertEqual(int(gate["env"]["MIN_FREE_GIB"]), sum(image[2] for image in IMAGES.values()))
        self.assertLess(gate["steps"].index(step_by_id(gate, "disk")),
                        gate["steps"].index(step_by_id(gate, "anonymous_smoke")))

    def test_pr_only_builds_test_target_and_never_logs_in_or_publishes(self):
        for job in self.validate["jobs"].values():
            self.assertEqual([step["uses"].split("@")[0] for step in job["steps"] if "uses" in step],
                             ["actions/checkout", "docker/setup-buildx-action", "docker/build-push-action"])
            self.assertEqual(action_steps(job, "docker/build-push-action"), [step_by_id(job, "test")])
        ...
        for workflow in self.workflows():
            for kind in IMAGES:
                ...
                self.assertEqual(build, {
                    "context": "${{ env.IMAGE_CONTEXT }}", "target": "test", "platforms": "linux/amd64",
                    "no-cache": "true", "push": "false", "load": "false", "provenance": "false",
                })
    ...
    def test_publish_tests_before_login_runtime_push_digest_smoke_and_attestation(self):
        for kind in IMAGES:
            ...
            self.assertEqual(build["with"]["tags"], "${{ env.IMAGE_NAME }}:source-${{ steps.source.outputs.sha }}")
            self.assertEqual(job["outputs"], {"digest": "${{ steps.build.outputs.digest }}"})
            self.assertEqual(step_by_id(job, "smoke")["env"], {"DIGEST": "${{ steps.build.outputs.digest }}"})
            self.assertEqual(attest["with"], {
                "subject-name": "${{ env.IMAGE_NAME }}", "subject-digest": "${{ steps.build.outputs.digest }}",
    ...
    def test_runtime_metadata_records_checked_out_source_revision_and_pinned_base(self):
        for kind in IMAGES:
            ...
            self.assertLess(job["steps"].index(metadata), job["steps"].index(step_by_id(job, "test")))
            self.assertIn('python3 scripts/source_delta.py >> "$GITHUB_STEP_SUMMARY"', metadata["run"])
    ...
    def test_anonymous_gate_is_fresh_read_only_and_consumes_both_job_digests(self):
        ...
        self.assertEqual([step["uses"].split("@")[0] for step in gate["steps"] if "uses" in step],
                         ["actions/checkout"])
        serialized = json.dumps({"env": self.publish["env"], "job": gate})
        for forbidden in ("secrets.", "github.token", "GITHUB_TOKEN", "GH_TOKEN", "docker login",
                          "download-artifact", "cache-from", "self-hosted"):
            self.assertNotIn(forbidden, serialized)
        for kind, (_, image, _) in IMAGES.items():
            self.assertEqual(gate["env"][kind.upper() + "_IMAGE"], image)
            self.assertEqual(gate["env"][kind.upper() + "_DIGEST"], "${{ needs." + kind + ".outputs.digest }}")
        config = step_by_id(gate, "anonymous_config")
        self.assertLess(gate["steps"].index(config), gate["steps"].index(step_by_id(gate, "anonymous_smoke")))
...
class ExecutableGuardTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name, head, source, workflow_sha, success in cases:
                with self.subTest(name=name):
                    ...
                    result = run_shell(script, directory, {"SOURCE_SHA": source, "GITHUB_SHA": workflow_sha})
                    self.assertEqual(result.returncode == 0, success, result.stdout + result.stderr)
                    if success:
                        self.assertEqual(output.read_text(), f"sha={source}\n")
                    else:
    ...
    def test_every_disk_gate_enforces_bytes_on_each_storage_path_without_cleanup(self):
        ...
        for workflow in (self.validate, self.publish):
            for kind, job in workflow["jobs"].items():
                ...
                for low_path in (None, *paths):
                    with self.subTest(kind=kind, low_path=low_path):
                        ...
                        with mock.patch.dict(os.environ, {
                            ...
                            "GITHUB_WORKSPACE": paths[1], "RUNNER_TEMP": paths[2],
                        }), mock.patch("shutil.disk_usage", side_effect=disk_usage), contextlib.redirect_stdout(io.StringIO()):
                            if low_path is None:
    ...
    def test_metadata_requires_digest_pinning_and_records_actual_revision(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for base, accepted in ((pinned_base, True), ("docker.io/example/runtime:latest", False),
                                   ("docker.io/example/runtime@sha256:abcd", False)):
                with self.subTest(base=base):
                    (context / "Dockerfile").write_text(f"FROM {base} AS upstream\nFROM upstream AS test\n")
                    output.unlink(missing_ok=True)
                    summary.unlink(missing_ok=True)
                    with mock.patch.dict(os.environ, {
                        "IMAGE_CONTEXT": str(context), "IMAGE_NAME": IMAGES["gateway"][1],
                        "SOURCE_URL": "https://github.com/example/repo", "SOURCE_SHA": "b" * 40,
                        ...
                        if accepted:
                            ...
                            self.assertEqual(output.read_text(), f"base_image={pinned_base}\nbase_digest={DIGEST}\n")
...
'''
    ...
    def test_publisher_smokes_exact_build_digest_not_source_tag(self):
        for kind, (_, image, _) in IMAGES.items():
            with self.subTest(kind=kind):
                ...
                result = run_shell(script, self.root, dict(self.env, IMAGE_NAME=image, DIGEST=DIGEST, SMOKE_KIND=kind))
                self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                self.assertEqual([(record["command"], record["args"]) for record in self.commands()], [
    ...
    def test_publisher_rejects_missing_malformed_or_tag_outputs_without_pulling(self):
        for digest in ("", "latest", "source-" + "a" * 40, "sha256:abcd", DIGEST + "\n"):
            with self.subTest(digest=digest):
                ...
                result = run_shell(script, self.root, dict(self.env, IMAGE_NAME=IMAGES["gateway"][1],
                                                         DIGEST=digest, SMOKE_KIND="gateway"))
    ...
    def anonymous_env(self):
        ...
        (inherited / "config.json").write_text('{"auths":{"ghcr.io":{"auth":"NOT_A_REAL_CREDENTIAL"}}}')
        env = dict(self.env, DOCKER_CONFIG=str(inherited), DOCKER_AUTH_CONFIG="unwanted auth",
                   DOCKER_CONTEXT="unwanted context", DOCKER_HOST="unwanted host")
        result = run_shell(step_by_id(gate, "anonymous_config")["run"], self.root, env)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        env.update(line.split("=", 1) for line in (self.root / "environment").read_text().splitlines())
        ...
        env.update(GATEWAY_IMAGE=IMAGES["gateway"][1], QWEN_IMAGE=IMAGES["qwen"][1],
                   GATEWAY_DIGEST=DIGEST, QWEN_DIGEST=OTHER_DIGEST)
        return env
    ...
    def test_anonymous_pull_uses_empty_config_and_both_independent_digest_outputs(self):
        ...
        self.assertEqual([(record["command"], record["args"]) for record in self.commands()], [
            ...
            ("python3", ["scripts/smoke_image.py", "gateway", IMAGES["gateway"][1] + "@" + DIGEST]),
            ("docker", ["pull", IMAGES["qwen"][1] + "@" + OTHER_DIGEST]),
            ("python3", ["scripts/smoke_image.py", "qwen", IMAGES["qwen"][1] + "@" + OTHER_DIGEST]),
        ])
    ...
    def test_anonymous_gate_rejects_either_invalid_digest_before_pulling(self):
        ...
        for kind in IMAGES:
            with self.subTest(kind=kind):
                result = run_shell(script, self.root, dict(env, **{kind.upper() + "_DIGEST": "latest"}))
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(self.commands(), [])

    def test_failed_pull_or_smoke_blocks_publish_and_anonymous_gate(self):
        anon_env = self.anonymous_env()
        cases = [(step_by_id(self.publish["jobs"]["anonymous_pull"], "anonymous_smoke")["run"], anon_env)]
        for kind, (_, image, _) in IMAGES.items():
            cases.append((step_by_id(self.publish["jobs"][kind], "smoke")["run"],
        ...
        for script, env in cases:
            for command in ("docker", "python3"):
                with self.subTest(script=script.splitlines()[-1], command=command):
                    ...
                    self.assertFalse((self.root / "summary").exists(), "Failure must not record a successful verification")
